...
describe(OfxImageEffectHandle effect)
{
  ...
  gPropHost->propSetString(effectProps, kOfxPropLabel, 0, "AI Plugin Renderer");
  gPropHost->propSetString(effectProps, kOfxImageEffectPluginPropGrouping, 0, "DeepMake");
...
{
  // std::string msg = "pluginMain: " + action;
  LogInfo("pluginMain: " + std::string(action));
...
{       
  ...
  1,
  "com.deepmake.ofx.ae.ai.renderer",
  1,
  0,
  setHostFunc,
  pluginMain
...
{       
  int numOfFilters = VideoFilterManager::instance().getNumOfFilters();
  LogInfo("OfxGetNumberOfPlugins" + std::to_string(numOfFilters) + "\n");
  return numOfFilters;
}
